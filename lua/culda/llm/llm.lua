...
"Apply the instructions only on the selected code. Consider the user input first, then the code comments. Generate code to replace selection only. Think step by step. If you must speak, do so in comments. Generate valid code only."
...
function M.setup(opts)
  ...
  -- collect user input and add to opts
  vim.api.nvim_create_user_command("POA", function(opts)
...
function M.prompt_with_input(opts)
  vim.ui.input({ prompt = "Prompt: " }, function(args)
    if args then
      opts.args = args
      M.prompt(opts)
...
function M.prompt(opts)
  ...
  -- Ensure opts.args is preserved
  if opts.args and opts.args ~= "" then
    prompt = "User input: " .. opts.args .. "\n"
  end

  local visual_lines = M.get_selection()
  if visual_lines then
    local selection = table.concat(visual_lines, "\n")
    if replace then
    ...
    prompt = prompt .. get_lines({ all = false })
  end
